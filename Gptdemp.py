...
def meal_planner_prompt(diet_preference, snack_preference, meal_amount_preference, other_preference):
    prompt = f'''
Create a meal plan for seven days, where for each day there is only {meal_amount_preference} meal no more each day, so for monday {meal_amount_preference} and so on. 
...
Use this information only, and only give back the recipes.
    '''
...
def recipe_prompt(recipe, portions):

    difficulty_of_recipe_prompt = f''''
...
Level 5: Difficult. For those looking for a challenge! These recipes usually contain unusual ingredients and require advanced cooking skills or elaborate preparations. 
    ...
    '''
    amount_of_time_for_recipe_prompt = f'''
...
Give me a recipe for {recipe}, that will feed {portions} people, include the price of each ingredient
    '''
    recipe_pic = respond_picture(
        f"Realistic picture of this recipe {recipe} on a white table, with a white plate with the food on it")

    assistant_answer_recipe = respond_bot(recipe_prompt)
    assistant_answer_difficulty = respond_bot(difficulty_of_recipe_prompt)
    assistant_answer_time = respond_bot(amount_of_time_for_recipe_prompt)
    return assistant_answer_recipe, assistant_answer_difficulty, assistant_answer_time, recipe_pic
...
Write a step by step exercise guide for {exercise}, with {weights}  
    '''
    exercise_pic = respond_picture(
        f"Show me a person doing this exercise {exercise}")
...
You provide personalized guidance, tips, and recommendations based on their specific needs, only answer health, fitness and personal questions. Do not answer questions if they are not related
to the users health.
...
def limit_message_history(message_history):
    ...
    count = sum(
        1 for message in message_history if message['role'] == 'user' or message['role'] == 'assistant')
    if count >= 9:
...
def respond_chatbot(message, chat_history, emoji, name):
    ...
    chat_history.append(
        (f"{str(emoji)}: " + message, " 🏋️‍♂️: " + bot_message))
    time.sleep(2)
    return "", chat_history
...
def check_name(name_textbox):
    ...
    if re.match(name_pattern, name_textbox) is None:
        raise gr.Error(
            "Type your name in the correct format: John Doe, John Adam Doe, or John")
...
def check_birth_date(birth_date_textbox):
    ...
    if re.match(age_pattern, birth_date_textbox) is None:
        raise gr.Error(
            "Type your birth date in the correct format: dd.mm.yyyy")
...
def check_weight(weight_textbox):
    ...
    if weight <= 30 or weight >= 200:
        raise gr.exceptions.Error(
            "Invalid weight, please type a number between 30 Kg and 200 Kg")
...
def check_height(height_textbox):
    ...
    if height <= 140 or height >= 200:
        raise gr.exceptions.Error(
            "Invalid height, please type a number between 140 cm and 200 cm")
...
with gr.Blocks(theme=gr.themes.Soft(), title="Health Assistant", css=".footer {display:none !important}") as demo:
    ...
    column1 = gr.Column(scale=2, min_width=600,
                        variant='compact', visible=True)
    column2 = gr.Column(scale=2, min_width=900,
                        variant='compact', visible=False)

    with column1:

        sex = gr.Dropdown(["Male", "Female", "Undefined"], label="Gender")
        birth_date = gr.Textbox(label="Birth Date", placeholder="dd.mm.yyyy")
        name = gr.Textbox(label="Name", placeholder="John Doe")
        weight = gr.Textbox(label="Weight", placeholder="Kg")
        height = gr.Textbox(label="Height", placeholder="cm")
        ...
        btn_1.click(check_all, inputs=[
                    name, birth_date, height, weight], outputs=[column2, column1])
    ...
    with column2:
        with gr.Blocks():
            with gr.Tab("Chatbot"):
                ...
                msg = gr.Textbox()
                msg.submit(respond_chatbot, [
                           msg, chatbot, user_emoji, name], [msg, chatbot])
            ...
            with gr.Tab("Meal planner"):
                ...
                snack_amount = gr.CheckboxGroup(
                    ["None", "1-2 snacks", "2-3 snacks", "3+ snacks"], label="Snack amount", info="How many snacks do you prefer?")

                meal_amount = gr.CheckboxGroup(
                    ["1 meal", "2 meals", "3 meals", "4+ meals"], label="Meal amount", info="How many meals do you prefer?")

                other_factors_meal = gr.Textbox(
                    label="Any other important factors to consider, for your meal plan?")

                meal_planner_output = gr.TextArea()
                btn_2 = gr.Button(label="Submit", variant="primary")
                btn_2.click(fn=meal_planner_prompt, inputs=[
                            diet_preference, snack_amount, meal_amount, other_factors_meal], outputs=meal_planner_output)
            ...
            with gr.Tab("Workout routine planner"):
                ...
                other_factors_workout = gr.Textbox(
                    label="Any other important factors to consider, for your workout plan")
            ...
            with gr.Tab("Recipe"):
                ...
                portion_amount = gr.Dropdown(
                    ["1", "2", "3", "4", "5", "6"], label="Portions")
                ...
                recipe_out = gr.TextArea(
                    label="The recipe:", scroll_to_output=True)

                recipe_picture = gr.Image(shape=[256, 256])
                btn_4.click(fn=recipe_prompt, inputs=[recipe, portion_amount], outputs=[
                            recipe_out, difficulty_out, time_out, recipe_picture])

            with gr.Tab("Exercise"):

                exercise = gr.Textbox(
                    label="What exercise do you want to know more about")
                weight_amount = gr.Dropdown(
                    ["Wights", "No wights"], label="Are you using weights")

                btn_4 = gr.Button(label="Submit", variant="primary")

                exercise_tips = gr.TextArea(
                    label="How to:", scroll_to_output=True)

                exercise_picture = gr.Image(shape=[256, 256])
                btn_4.click(fn=exercise_prompt, inputs=[exercise, weight_amount], outputs=[
                            exercise_tips, exercise_picture])
